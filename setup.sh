...
prompt_secret() {
  ...
  # Intentionally visible so the attendee can verify that a pasted test key is complete.
  # Never share, commit, screenshot, or paste the key into chat.
...
ensure_link_cli_mpp_support() {
  ...
  printf '\nYour installed Link CLI (%s) is too old for this Machine Payments demo.\n' "$version" >&2
  printf 'The demo requires link-cli mpp pay to support both --test and --context.\n\n' >&2
...
register_catalog() {
  ...
  TMP_RESPONSE_FILE="$(mktemp "${TMPDIR:-/tmp}/mpp-demo-catalog-response.XXXXXX")"
